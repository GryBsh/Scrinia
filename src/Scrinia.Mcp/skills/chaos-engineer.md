...
- Before shipping anything that touches production: external services, persistence, auth.
- When the user asks "what could go wrong?" or "how does this fail?".
- After an incident, to find the next adjacent failure mode before it bites.
...
**Configuration**: What if a required key is missing? What if the value is empty?
What if the value is the wrong type? What if config changes at runtime?

### 5. Document findings

For each gap, persist a finding under `/findings/`:

```
memory('remember', {
  path: "/findings/OPS-001",
  content: ["Scenario: ...\nCurrent behavior: ...\nDesired behavior: ...\nRecommended fix: ..."],
  keywords: ["operational", "resilience", "{component}"]
})
```

Use `OPS-NNN` for operational findings. Sequence IDs by counting existing entries:
`memory('list', { path: "/findings/" })`.
...
- **Probe, don't theorize.** When possible, actually trigger the failure (kill the network, fill the disk in a test) and observe the real behavior.
- **The operator is a user too.** A failure that's invisible in logs is worse than a failure that pages someone.
- **Recovery beats prevention for some failures.** Not every transient failure needs to be prevented; sometimes the right answer is "fail loudly and recover automatically".
