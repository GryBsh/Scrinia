...
```
                  ...
                  +---------------------------------------------+
                  |              scri (CLI + MCP)                |
                  |  ScriniaCommands | ScriniaMcpTools (3)       |
                  |                  | ScriniaProjectTools (6)   |
                  +--------+------------+------------------------+
                           |            |
              +------------+            +-----+
              |                               |
...
Scrinia.sln (11 projects)
  src/
    ...
    Scrinia.Mcp/                     MCP tool definitions (9 tools: 3 memory + 6 planning)
  ...
  tests/
    Scrinia.Tests/                   821 tests (CLI + MCP + Core + Embeddings + Planning)
    Scrinia.Server.Tests/            63 tests (HTTP API)
...
- **Mcp** depends only on Core and `ModelContextProtocol`. Contains two sealed tool classes: `ScriniaMcpTools` (3 memory tools) and `ScriniaProjectTools` (6 planning tools), plus `PlanningJsonContext` for trimming safety
