...
public sealed class PlanningHelperTests : IDisposable
{
    ...
    public async Task PlanStatus_NoTasks_ShowsZeroProgress()
    {
        ...
        ResponseParser.Parse(result).Content.Should().Contain("Progress: 0%", because: "no tasks decomposed = 0% progress");
    }
    ...
    public async Task PlanStatus_AllTasksComplete_Shows100Percent()
    {
        ...
        // Complete user task + all auto-injected gate tasks (discover names dynamically)
        ...
        foreach (var gateSuffix in new[] { "qa-gate", "self-reflector-gate", "evolutionary-gate", "cartographer-gate", "march-gate" })
        {
            var entry = allEntries.FirstOrDefault(e => e.Name.Contains(gateSuffix));
            if (entry is not null)
                await ScriniaProjectTools.TaskComplete($"task:{entry.Name}", "completed", CancellationToken.None);
        }

        var result = await ScriniaProjectTools.PlanStatus(CancellationToken.None);
        ResponseParser.Parse(result).Content.Should().Contain("Progress: 100%", because: "all tasks complete = 100%");
    }
    ...
    public async Task PlanStatus_PartialCompletion_ShowsCorrectPercent()
    {
        ...
        ResponseParser.Parse(result).Content.Should().Contain("Progress: 14%", because: "1 of 7 tasks complete (2 user + 5 gates) = 14%");
    }
    ...
    public async Task PlanStatus_ReturnsAllStateFields()
    {
        ...
        parsed.Content.Should().Contain("Project:");
        parsed.Content.Should().Contain("Progress:");
        parsed.Content.Should().Contain("Blockers:");
        parsed.Content.Should().Contain("Next:");
    }
    ...
    public async Task TaskNext_ReturnsOnlyCurrentWave()
    {
        ...
        var result = await ScriniaProjectTools.TaskNext("01", CancellationToken.None);
        var parsed = ResponseParser.Parse(result);
        parsed.Content.Should().Contain("Wave 1", because: "should return wave 1 first");
        parsed.Content.Should().NotContain("wave2", because: "wave 2 tasks should not appear yet");
    }
    ...
    public async Task TaskNext_AfterWave1Complete_ReturnsWave2()
    {
        ...
        await ScriniaProjectTools.PlanTasks("01",
            "## Task 01\nWave: 1\nDepends on: none\nAction: wave1\nAcceptance criteria:\n- done\n\n" +
            "## Task 02\nWave: 2\nDepends on: 01-1-01\nAction: wave2\nAcceptance criteria:\n- done", CancellationToken.None);
        await ScriniaProjectTools.TaskComplete("task:01-1-01", "done", CancellationToken.None);

        var result = await ScriniaProjectTools.TaskNext("01", CancellationToken.None);
        ResponseParser.Parse(result).Content.Should().Contain("wave2", because: "wave 1 complete, wave 2 should now appear");
    }
    ...
    public async Task PlanVerify_AllTasksComplete_ReturnsAllPass()
    {
        ...
        var checklist = await ScriniaProjectTools.PlanVerify("01", cancellationToken: CancellationToken.None);
        var checklistParsed = ResponseParser.Parse(checklist);
        checklistParsed.Content.Should().Contain("Verification Checklist");
        checklistParsed.Content.Should().Contain("All tasks complete");
        ...
        var result = await ScriniaProjectTools.PlanVerify("01", evidence: "PASS: All tasks complete — 1 passed, 0 failed", cancellationToken: CancellationToken.None);
        ResponseParser.Parse(result).Content.Should().Contain("ALL_PASS");
    }
    ...
    public async Task CarryForwardSite_WritesComputedProgress_NotStaleValue()
    {
        ...
        await ScriniaProjectTools.ProjectInit("Goals: test carry-forward", CancellationToken.None);
        await ScriniaProjectTools.PlanRequirements("## v1\n- REQ-01: Test", CancellationToken.None);
        await ScriniaProjectTools.PlanTasks("01",
            "## Task 01\nWave: 1\nDepends on: none\nAction: first\nAcceptance criteria:\n- done\n\n" +
            "## Task 02\nWave: 1\nDepends on: none\nAction: second\nAcceptance criteria:\n- done",
        ...
        await ScriniaProjectTools.ConcernAdd("Test concern for carry-forward", "low", "01",
    ...
    public async Task Restore_ShowsComputedProgress_NotStaleStateProgress()
    {
        ...
        var parsed = ResponseParser.Parse(restoreResult);
