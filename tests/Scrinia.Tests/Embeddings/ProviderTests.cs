...
public class ProviderTests
{
    ...
    [Fact]
    public void Model2Vec_IsDefaultProvider()
    {
        var options = new EmbeddingOptions();
    ...
    [Fact]
    public void Model2Vec_FactoryFallsBackWhenModelNotDownloaded()
    {
        var options = new EmbeddingOptions { Provider = "model2vec" };
        // Use a nonexistent dir so model won't be found
        var provider = EmbeddingProviderFactory.Create(options, Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString()), Logger);
        provider.Should().BeOfType<NullEmbeddingProvider>();
    }
    ...
    [Fact]
    public void Factory_NoneProvider_ReturnsNull()
    {
    ...
    [Fact]
    public void Factory_UnknownProvider_ReturnsNull()
    {
